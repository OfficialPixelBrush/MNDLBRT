...
unsigned int mode, format, juliaMode, funcNum, t, maxPower;
float scaleX, scaleY;
...
/* complex number sine */
struct complexNumber complexSin(struct complexNumber x) {
	struct complexNumber result;
	result.real = sin(x.real)*cosh(x.imaginary);
	result.imaginary = cos(x.real)*sinh(x.imaginary);
	return result;
}
...
void assignDeadColorArray() {
	...
	for (i = 0; i<6000; i=i+3) {
		deadColorArray[i/3].r = deadColorArrayRGB[i];
		deadColorArray[i/3].g = deadColorArrayRGB[i+1];
		deadColorArray[i/3].b = deadColorArrayRGB[i+2];
	}
}
...
void drawBasedOnMode(int i, int x, int y, int blank, struct complexNumber z) {
	...
	if (imageSave != 0) {
		...
		drawpixel(bufAdr, x, y, finalCol.r, finalCol.g, finalCol.b);
	}
}
...
void drawSetInMode() {
	...
	if (imageSave != 0) {
		int i;
		char iString[255];
		char xCoordString[255];
		char yCoordString[255];
		char ZoomCoordString[255];
		char ending[4] = ".bmp";
		filename[0] = 0;
		sprintf(iString, "%d", (int)t);
		snprintf(xCoordString, 32, "%8.24g", xCoord);
		snprintf(yCoordString, 32, "%8.24g", yCoord);
		sprintf(ZoomCoordString, "%.2lf", Zoom);
		strcat(filename,iString);
		strcat(filename,"iter_X");
		strcat(filename,xCoordString);
		strcat(filename,"_Y");
		strcat(filename,yCoordString);
		strcat(filename,"_Zoom");
		strcat(filename,ZoomCoordString);
	...
	if (xSize > ySize) {
		Xaspect = ((float)xSize)/((float)ySize);
		Yaspect = 1;
	} else {
		Xaspect = 1;
		Yaspect = ((float)ySize)/((float)xSize);
	}
	...
	for (y = 0; y < ySize; y++) {
		...
		for (x = 0; x < xSize; x++) {
			...
			if (juliaMode) {
				...
				z.real = mapRange(0, xSize, xCoord-(20*Xaspect)/Zoom, xCoord+(20*Xaspect)/Zoom, x);
				z.imaginary = mapRange(0, ySize, yCoord+(20*Yaspect)/Zoom, yCoord-(20*Yaspect)/Zoom, y);
			} else {
				c.real = mapRange(0, xSize, xCoord-(20*Xaspect)/Zoom, xCoord+(20*Xaspect)/Zoom, x);
				if (imageSave) {
					c.imaginary = mapRange(0, ySize, yCoord-(20*Yaspect)/Zoom, yCoord+(20*Yaspect)/Zoom, y);
				} else {
					c.imaginary = mapRange(0, ySize, yCoord+(20*Yaspect)/Zoom, yCoord-(20*Yaspect)/Zoom, y);
				}
				z = c;
			...
			for (i = 0; i < t; i++) {
				...
				switch(funcNum) {
					case 1:
						...
						break;
					case 2:
						z = complexAdd(complexPower(z,mndlPow),c);
						break;
					case 3:
						z.real = absCustom(z.real);
						z.imaginary = absCustom(z.imaginary);
						z = complexAdd(complexPower(z,mndlPow),c);
						break;
					case 4:
						z = complexDivide(complexAdd(complexPower(complexDivide(complexPower(z,2),c),4),c),c);
						break;
					case 5:
						z = complexSin(complexAdd(complexPower(z,mndlPow),c));
						break;
				}
				
				/* Regular Escape */
				if ((z.real*z.real + z.imaginary*z.imaginary) > 1024) {
					drawBasedOnMode(i,(int)x+(SCREEN_WIDTH-xSize)/2,(int)(y+yStart)+(SCREEN_HEIGHT-ySize)/2, 1, z);
					break;
				}
			}
			if (i==t) {
				drawBasedOnMode(i,(int)x+(SCREEN_WIDTH-xSize)/2,(int)(y+yStart)+(SCREEN_HEIGHT-ySize)/2, 0, z);
			}				
		}
	}
}
...
int main(int argc, char **argv)
{
	...
	/* start */
	consoleSelect(&topScreen);
	printf("\nMandelbrot Set (in C for Nintendo 3DS)\n");
	printf("Written by PixelBrushArt (2021)\n");
	printf("v8.5\n");
	
	printf("\n- Function -");
	printf("\nBubbles (1), Mandelbrot (2), Burning Ship (3), may crash (4), Mandelbrot Sine (5): ");
	...
	if ((funcNum == 2) || (funcNum == 3) || (funcNum == 5)) {
		printf("Power (Default: 2)\n");
		swkbdInit(&swkbd, SWKBD_TYPE_NUMPAD, 1, 8);
		button = swkbdInputText(&swkbd, mybuf, sizeof(mybuf));
		consoleInit(GFX_TOP, &bottomScreen);
		mndlPow = atoll(mybuf);
	...
	printf("Juliaset Mode (Default: 0)\n");
	swkbdInit(&swkbd, SWKBD_TYPE_NUMPAD, 1, 8);
	button = swkbdInputText(&swkbd, mybuf, sizeof(mybuf));
	consoleInit(GFX_TOP, &bottomScreen);
	juliaMode = atoll(mybuf);
	if (juliaMode > 1) {
		juliaMode = 1;
	}
	...
	if (juliaMode) {
		...
		consoleInit(GFX_TOP, &bottomScreen);
		cReal = atof(mybuf);
		printf("Imaginary of c:\n");
		swkbdInit(&swkbd, SWKBD_TYPE_NORMAL, 3, -1);
		button = swkbdInputText(&swkbd, mybuf, sizeof(mybuf));
		consoleInit(GFX_TOP, &bottomScreen);
		cImaginary = atof(mybuf);
	}
	
	// Get location 
	/*
	printf("x Coordinate (Default: 0):\n");
	swkbdInit(&swkbd, SWKBD_TYPE_NORMAL, 3, -1);
	button = swkbdInputText(&swkbd, mybuf, sizeof(mybuf));
	consoleInit(GFX_TOP, &bottomScreen);
	xCoord = atof(mybuf);
	
	printf("y Coordinate (Default: 0):\n");
	swkbdInit(&swkbd, SWKBD_TYPE_NORMAL, 3, -1);
	button = swkbdInputText(&swkbd, mybuf, sizeof(mybuf));
	consoleInit(GFX_TOP, &bottomScreen);
	yCoord = atof(mybuf);
	
	printf("Zoom (Default: 10):\n");
	swkbdInit(&swkbd, SWKBD_TYPE_NORMAL, 3, -1);
	button = swkbdInputText(&swkbd, mybuf, sizeof(mybuf));
	consoleInit(GFX_TOP, &bottomScreen);
	Zoom = atof(mybuf);
	
	printf("Iterations (Default: 25):\n");
	swkbdInit(&swkbd, SWKBD_TYPE_NUMPAD, 1, 8);
	button = swkbdInputText(&swkbd, mybuf, sizeof(mybuf));
	consoleInit(GFX_TOP, &bottomScreen);
	t = atoll(mybuf);*/
	...
	while( aptMainLoop() ) {
		...
		/* Regular Chars */
		if (kDown & KEY_R) { /* increase resolution, Right Shoulder */
			res /= 2;
			clearScreen();
			goto draw;
		}
		if (kDown & KEY_L) { /* increase resolution, Right Shoulder */
			res *= 2;
			clearScreen();
			goto draw;
		}
		...
		if (kDown & KEY_DDOWN) { /* down */
			yCoord = yCoord-2/Zoom;
			goto draw;
		}
		if (kDown & KEY_DLEFT) { /* left */
			xCoord = xCoord-2/Zoom;
			goto draw;
		}
		if (kDown & KEY_DRIGHT) { /* right */
			xCoord = xCoord+2/Zoom;
			goto draw;
		}
		if (kDown & KEY_Y) { /* zoom in, Y */
			Zoom = Zoom*1.25;
			goto draw;
		}
		if (kDown & KEY_B) { /* zoom out, B */
			Zoom = Zoom/1.25;
			goto draw;
		}
		if (kDown & KEY_X) { /* increase iterations, X */
			t = t+25;
			goto draw;
		}
		if (kDown & KEY_A) { /* decrease iterations, A */
			t = t-25;
			if (t < 1) {
				t = 25;
			}
